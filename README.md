...
1.8.14/1.8.15 内容：题库 v2（脑筋急转弯 312 条优化版，107 条重写）、收藏室独立页（爱心交互、类型筛选、持久化）、首页改版（每日一言主卡片、换一句/抽签独立按钮、四入口卡片）、签运持久化（今日签跨重启保留、遮罩页今日签/近签展示）、宜模板 12→64 条、「灵光」全量移除、修复收藏删除后复活与收藏类型退化两个 P1。1.8.13：修复 P1「退出测试直接离开应用」+ P2 死代码清理；1.8.12：纯架构清理（`index.jsc` −68.5%）。
