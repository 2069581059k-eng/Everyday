...
当前共同开发基准为 **1.8.19**（正式版，PR #10 合并 `e88f1f9`，tag `v1.8.19` → 实现提交 `3f73efd`，真机未验），WorkBuddy、Trae Code 与 Codex 后续请从最新 `main` 继续开发。[下载 1.8.19 正式版安装包](https://github.com/2069581059k-eng/DAILY-NOTE/releases/tag/v1.8.19)（BIN/RPK 各 710,972 B，SHA-256 `433787d0c8ca809914276d81a6054fae596698ea60f480ff84ed93ae6b8f4a17`）。

1.8.19 内容：收藏室交互对齐知识页（移除「继续」改点击正文翻页、正文「答案：」前缀与旧存档重复标题清洗、「分类 · 已收录」重复副标题移除、正文框扩容 226px）、右滑退出提示移至屏幕中上方、星象遮罩移除今日签/近签显示且插图直显当前星座素材（随切换联动，映射收敛共享模块）、用户提供新设计素材 11 枚替换入库。1.8.18：全局视觉改版「东方纸韵 · 极简治愈」、星象结果页 12 枚星座插画、P1 左缘触摸死区修复（64px 透明命中层 + 面板下沉）。1.8.14–1.8.17：题库 v2、收藏室独立页（筛选/双布局详情）、签运持久化、右滑交互重构、知识页点击翻页、DAILY NOTE 更名。
