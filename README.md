...
当前共同开发基准为 **1.8.23**（正式版，PR #14 合并 `ba47390`，tag `v1.8.23` → 实现提交 `5b2aaa7`，真机未验），WorkBuddy、Trae Code 与 Codex 后续请从最新 `main` 继续开发。[下载 1.8.23 正式版安装包](https://github.com/2069581059k-eng/DAILY-NOTE/releases/tag/v1.8.23)（BIN/RPK 各 1,303,873 B，SHA-256 `0ece95de3128b0beac872d55fbc86d0581a532e2f159e184737b9aa9234a21c1`）。

1.8.23 内容：新增倒数日/纪念日模块（管理页步进选择增删、内置法定节日倒数、日历页标记与入口、首页今日日历卡片副标题联动），验收防线加强（strict 点击 + 截图内容下限断言），内联构建去重修复。1.8.22：应用图标恢复 1.8.14 经典版（紫蓝魔法书+金色星光），imgs 其余素材映射不变。1.8.21：素材统一 imgs 目录供图——应用图标换新（星光轨道，1.8.22 已撤销）+ 今日日历导航图标补全。1.8.20：右滑退出提示改羊皮纸主题卡片并移至屏幕最上方。1.8.19：收藏室交互对齐知识页（点击正文翻页/正文清洗/去重复副标题/正文框扩容）、星象遮罩移除今日签近签且插图直显当前星座素材、新设计素材 11 枚。1.8.18：全局视觉改版「东方纸韵 · 极简治愈」、星象结果页 12 枚星座插画、P1 左缘触摸死区修复（64px 透明命中层 + 面板下沉）。1.8.14–1.8.17：题库 v2、收藏室独立页（筛选/双布局详情）、签运持久化、右滑交互重构、知识页点击翻页、DAILY NOTE 更名。
