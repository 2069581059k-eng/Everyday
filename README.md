...
`main` 另含**未发布**的 1.8.13（Trae Code）：修复 P1「退出测试直接离开应用」（`router.back()` 改 `router.replace` 返回主页，与知识/日历/结果页一致），并完成 P2 死代码清理（`zodiac-scoring.js` 删除未用的 `getZodiacByDate`；`date.js` 按消费者拆分出 `moon.js`/`holiday.js`，各页内联后不再携带无用代码）。模拟器验收全链路通过：退出测试后回到首页且应用仍在前台（画面与首页哈希完全一致），首页渲染与 1.8.11 逐像素一致。尚未创建 Release。
