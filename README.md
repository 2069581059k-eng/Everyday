...
当前共同开发基准为 **1.8.18**（正式版，PR #9 合并 `da83a94`，tag `v1.8.18` → 实现提交 `1921c40`，真机未验），WorkBuddy、Trae Code 与 Codex 后续请从最新 `main` 继续开发。[下载 1.8.18 正式版安装包](https://github.com/2069581059k-eng/DAILY-NOTE/releases/tag/v1.8.18)（BIN/RPK 各 1,059,874 B，SHA-256 `b3c0c91695c6e173a39d789653d6e6c764c1a5852b31bd5e8f8df3c19cada700`）。

1.8.18 内容：全局视觉改版「东方纸韵 · 极简治愈」（设计令牌六页统一：纸底 #f2eee5 / 卡片 #fffdf8 / 品牌红 #a83b2d 等；统一页面头部；设计稿导航图标与主题插图；应用图标换新）、星象结果页接入 12 枚星座插画、修复 P1 收藏详情页回退箭头失效（左缘 x<40 触摸死区，64px 透明命中层 + 面板下沉零重叠）。1.8.14–1.8.17：题库 v2、收藏室独立页（筛选/双布局详情）、签运持久化、右滑交互重构（首页二次确认退出、子页返回上一级）、知识页点击翻页、DAILY NOTE 更名。
