...
当前共同开发基准为 **1.8.15**（正式版，PR #6 合并 `b6f5ead`，tag `v1.8.15`，真机验证通过），WorkBuddy、Trae Code 与 Codex 后续请从最新 `main` 继续开发。[下载 1.8.15 正式版安装包](https://github.com/2069581059k-eng/Everyday/releases/tag/v1.8.15)（BIN/RPK 各 1,069,951 B，SHA-256 `a53bc716b83786c8afd672165a38709c35e1e86a0ad0a8ac5a6a5a46bf7e3738`）。

1.8.14/1.8.15 内容：题库 v2（脑筋急转弯 312 条优化版，107 条重写）、收藏室独立页（爱心交互、类型筛选、持久化）、首页改版（DAILY NOTE主卡片、换一句/抽签独立按钮、四入口卡片）、签运持久化（今日签跨重启保留、遮罩页今日签/近签展示）、宜模板 12→64 条、「灵光」全量移除、修复收藏删除后复活与收藏类型退化两个 P1。1.8.13：修复 P1「退出测试直接离开应用」+ P2 死代码清理；1.8.12：纯架构清理（`index.jsc` −68.5%）。
