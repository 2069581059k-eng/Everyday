...
// 唤醒后截图：模拟器首次启动/闲置时屏幕可能处于熄屏（纯黑），需发 WAKEUP 并重试
async function shotAwake(client, filename, adbSerial) {
  for (let attempt = 1; attempt <= 3; attempt++) {
    const len = await shot(client, filename)
    if (len >= 5000) return len
    runtimeLog.push(`截图过小(${len}B)，尝试唤醒第 ${attempt} 次`)
    try { run(adbPath, ['-s', adbSerial, 'shell', 'input', 'keyevent', 'KEYCODE_WAKEUP'], 10000) } catch {}
    await wait(2500)
  }
  return 0
}
...
async function main() {
  ...
  try {
    ...
    await click(client, ...P.ztExit)               // 退出测试（router.back()）
    await wait(1800)
    await shotAwake(client, '19-exit-test.png', serial)
    const pidAfterExit = run(adbPath, ['-s', serial, 'shell', 'pidof', packageName], 10000).trim()
    runtimeLog.push(`退出测试后进程 pidof=${pidAfterExit || '(空，应用已退出)'}`)
    console.log(`退出测试后进程：${pidAfterExit || '(空，应用已退出)'}`)
    // 重新拉起应用，确认首页可再次正常渲染
    run(adbPath, ['-s', serial, 'shell', 'am', 'start', packageName], 15000)
    await wait(9000)
    await shotAwake(client, '20-final-home.png', serial)
